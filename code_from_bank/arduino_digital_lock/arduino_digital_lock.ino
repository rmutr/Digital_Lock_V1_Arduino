...
void setup() { 
  ...
  digitalWrite(PIN_RELAY_A, PNP_OFF); 
  digitalWrite(PIN_RELAY_B, PNP_OFF); 
  digitalWrite(PIN_RELAY_C, PNP_OFF); 
...
//-----------------------------------------------------------------------------EEPROM
 while(readEEPROM(EEPROM_I2C_ADDRESS, address) == 30)  address += 5;
...
//-----------------------------------------------------------------------------servo
  ...
  myservo.write(10); //open130 close10
  
  Serial.print("-> code : ");
  Serial.println(bt_pincode_str);   
} 
...
//----------------------------------------------------------------------------- 
 ...
 //----------------------------------------------------------------------------- 
            if(readEEPROM(EEPROM_I2C_ADDRESS, address) == 30)
              address += 5;
            if(readEEPROM(EEPROM_I2C_ADDRESS, address) < 30)
              writeEEPROM(EEPROM_I2C_ADDRESS, address, (readEEPROM(EEPROM_I2C_ADDRESS, address)+1));    
            for (int str=0; str < 4; str++)
              writeEEPROM(EEPROM_I2C_ADDRESS, (address+str+1), (bt_pincode_str.substring(str, (str+1)).toInt())); 
 //-----------------------------------------------------------------------------            
          } 
...
//----------------------------------------------------------------------------- 
  if (machine_run == 0) { 
    ...
    digitalWrite(PIN_RELAY_A, PNP_ON); 
    digitalWrite(PIN_RELAY_B, PNP_ON); 
    myservo.write(130); //open130 close10
  } 
